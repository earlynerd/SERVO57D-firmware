...
## 2026-08-22 — Add operator-acknowledged in-place fault recovery

- **Decision:** Firmware 0.29.0 / native protocol 1.11 adds `CLEAR_FAULTS` (`0x0203`) as the production operator acknowledgment and recovery command. STOP remains an authority-termination request and deliberately does not clear latches. An empty clear request returns schema-1 result, blocker, cleared-source, and remaining-source bitmaps; the project host exposes the same operation as `clear-faults`.
- **Recovery contract:** The operator command itself is sufficient assertion that the initiating condition is gone. Firmware does not require a healthy encoder sample, in-range current sample, released Right button, or another condition-specific proof before attempting recovery. It cancels pending command mailboxes, establishes the direct-GPIO all-low `ZERO` vector, restarts timer-synchronous ADC/DMA, rebuilds TIM3 plus the current-loop/predictor backend, resets estimator and alignment/torque/velocity/position operational state, clears pending rotor-runtime fault events, and acknowledges the supervisor into uncommanded `DIAGNOSTIC`. Ordinary fresh current and encoder production then restore `READY`; a persistent condition faults again through its normal monitor.
- **Recoverability contract:** No implemented initiating fault class is declared permanently unrecoverable merely because an earlier reset API was absent. Phase-current, ADC, PWM, current-loop deadline/internal/prediction, estimator, alignment, aligned-torque, velocity, position, and supervisor latches all have an in-place reset path. `blocked` describes a concrete failure to complete the ZERO/backend, runtime, or supervisor reset transaction coherently, not a permanent classification of the initiating source.
- **Preservation contract:** Recovery does not restore authority or resume a command. It preserves accepted motor alignment, persistent configuration, boot/reset cause, watchdog history, and retained panic evidence. If the estimator latch was cleared, position/velocity estimation reseeds from the next accepted encoder sample.
- **Why:** A following-error event correctly stopped the bridge but left the product in `FAULT`; the only public operation was STOP, which could terminate authority but had no latch-acknowledgment semantics. Power cycling was therefore the only way to continue even though every required subsystem already retained enough configuration to rebuild in place.
...
## 2026-08-22 — Separate controller feedback timing from predictor dispatch age

- **Decision:** Firmware 0.29.1 retains the controllers' 2,000 us feedback-timestamp interval, but permits the 20 kHz phase predictor through 3,000 us: statically greater than the controller boundary and no later than the independent encoder-production deadline. Native protocol 1.12 aligned-torque schema 2 appends rejection reason/age and maximum successful/configured ages without moving existing fields.
- **Why:** Flashed 0.29.0 recovered correctly, then twice faulted prediction despite healthy 1,000-1,014 us encoder intervals. Matching both limits ignored completion-to-PendSV dispatch latency.
- **Supersedes:** The matched 2 ms predictor clause in “Advance aligned electrical phase” and the paired-age wording in “Expose the motion search space.” Controller feedback and total-production limits remain unchanged.
- **Affects:** predictor/backend timing, aligned-torque telemetry, native protocol, host tooling, tests, and timing/limits/protocol/bring-up documentation
- **Validation:** Host, native C, and clean Debug/Release Arm builds pass; hardware reflash and the post-recovery motion gate remain open.

## 2026-08-22 — Publish microsecond time coherently across nested interrupts

- **Decision:** Firmware 0.29.2 retains the 1 kHz SysTick at priority 15 and publishes `timebase_micros()` through a monotonic interrupt-safe reconciliation point. A raw regression shorter than one 1,000 us SysTick period is the hardware-reload/software-epoch gap and receives the missing epoch; larger stale values clamp to the last publication. The raw epoch/fraction snapshot and the `LDREX`/`STREX` publication each have four-attempt bounds; a preempted lower-priority caller cannot overwrite a newer timestamp, and either exhausted retry returns the already published value without spinning.
- **Why:** Flashed 0.29.1 retained predictor rejection age `0xFFFFFE58`, exactly -424 us, while encoder production and every prior successful prediction age were healthy. SysTick exception entry had cleared `PENDSTSET`, but the priority-2 current ISR preempted before the priority-15 handler incremented `s_milliseconds`; the old software epoch was combined with the newly reloaded single-register `VAL` fraction.
- **Timing contract:** Current control continues to preempt timekeeping; no interrupt priority is raised, no interrupt is globally masked, and no caller waits for a lower-priority handler. The public microsecond counter preserves modulo-2^32 wrap semantics and cannot regress because of this one-tick epoch-service window. Millisecond uptime/deadline semantics remain unchanged.
- **Supersedes:** The assumption that pending-bit compensation alone made the SysTick epoch/fraction composition coherent. It does not weaken the 2 ms controller interval, 3 ms predictor/encoder-production deadlines, or any current, voltage, duty, motion, authority, or fault bound.
- **Affects:** firmware version 0.29.2, `timebase_micros`, current-loop/encoder timestamp consumers, native timing regressions, realtime architecture, build/bring-up state, and the predictor hardware gate
- **Validation:** The rebuilt native suite covers the observed -424 us regression, a 999 us missing epoch, the full-tick stale boundary, ordinary forward time, larger stale samples, and uint32 wrap. All 18 Python console tests pass. Clean post-link builds pass: Debug uses 58,664 bytes Flash and 7,480 bytes SRAM1; Release uses 52,948 bytes Flash and 7,480 bytes SRAM1, with no configuration-slot or SRAM2 allocation and the 240-byte debugger diagnostic ABI intact. Release disassembly contains the bounded exclusive-access publication. Hardware flash and alternating signed-motion confirmation remain open.

## 2026-08-22 — Make acceleration and cascade deadlines explicit contracts

- **Decision:** Firmware 0.29.3 / protocol 1.12 sets the aligned actuator's independent observed-acceleration shutdown to 512 rev/s²: twice the maximum 256 rev/s² inner reference slew and well above the filtered encoder's single-count step. Position, velocity, and torque layers must start from the same timestamp and duration; the outer position layer is evaluated first and owns normal deadline release.
