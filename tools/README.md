...
`motor_test.py` is the normal human-facing motor-development loop. It runs one
firmware-bounded rotating-current move, captures the commissioning stream and
...
`mks57d_rs485.py` is the lower-level native protocol motor console. It uses PySerial,
...
`analyze_current_loop.py` summarizes the slower commissioning stream's RMS
