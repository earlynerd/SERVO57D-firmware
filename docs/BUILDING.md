...
Firmware 0.20.0 is the converged current-regulated product firmware. It:
...
12. Parses native v1.4 COBS/CRC frames in foreground and replies to valid address-1 discovery, boot, raw/estimated encoder, and current-diagnostic requests, including live status and STOP while active.
...
18. Publishes firmware `0.20.0`, authoritative drive state, reset cause, retained panic, uptime, heartbeat, watchdog health, priority policy, self-test masks, raw encoder state, RS-485 transport state, native-protocol counters, and current-loop state through the unchanged 240-byte schema-5 `g_diagnostics` RAM record; estimator fields are presently on wire rather than appended to that debugger ABI.
...
Firmware 0.20.0 retains the 0.19.0 supervisor-authorized local hold-to-run and
...
hardware regression; estimator readiness-loss injection remains pending. Use
