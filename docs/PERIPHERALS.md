...
and continues it during current-loop operation. The current source candidate
for firmware 0.32.2 runs SPI1 at an 8 MHz target in mode 3; TIM6 releases one
registers-`0x03`-through-`0x05` four-byte CS window at 4 kHz. The observation
timestamp is captured when CS asserts at the start of that coherent acquisition
window, rather than after the CS-hold interval when the frame is published.
TIM7 bounds CS timing, DMA channels 2/3 transfer the frame, and PendSV defers
...
during a 5.97 RPM motor run. Those idle and energized timing regressions prove
the flashed 500 kHz / 1 kHz schedule. An informal 8 MHz / 4 kHz run is
encouraging evidence for the 0.32.2 source candidate, not formal qualification;
signal-integrity, timing, noise, and estimator/predictor validation remain.
