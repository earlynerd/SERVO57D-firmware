...
Status: firmware 0.37.1 / protocol 1.18 is the current source and flashed
...
| Encoder SPI clock | 8 MHz flashed setting | Exact divisor of the 32 MHz SPI1 peripheral clock; four-byte mode-3 DMA frame and 2 us CS setup/hold guards are retained | `spi1` | Firmware 0.37.1 signed torque and velocity gates ran with 250 us captured intervals and zero reported encoder errors; formal SCK/CS signal-integrity correlation, parity/status behavior, and long-run error counters remain open |
| Rotor/control release | 4 kHz, 250 us nominal | Implementation timing candidate with 16,000 core cycles between releases at 64 MHz | TIM6/TIM7, SPI1 DMA, PendSV, and `rotor_control_runtime` | Firmware 0.37.1's traced signed velocity gates held captured intervals to 250 us and prediction age to 174-456 us while every armed 20 kHz trace remained consecutive; formal PendSV WCET, higher-priority current-loop preemption, foreground load, and stack high-water evidence remain open |
...
| Accepted feedback interval | 2,000 us maximum | Timing policy for the nominal 4 kHz encoder schedule, allowing eight nominal release periods before rejection | `aligned_torque_controller` | Firmware 0.37.1 signed torque and velocity gates held captured intervals to 250 us; the final inactive check observed a 273 us historical maximum with no error or fault. Retain formal preemption and worst-case load measurement before tightening the 2,000 us bound |
| Fast electrical-phase prediction | Every 50 us; 3,000 us maximum observation age; 55 us measured output lead; observations through 20 rev/s | Timing contract: predict from regular-current DMA completion near 45 us to the 100 us PWM application boundary; the priority-1 guardian permits the intentional intervening update but faults a second update without new output | `electrical_phase_predictor` through `current_loop_backend` | Across five firmware 0.37.1 arms, prediction age was 174-456 us, trigger-to-DMA time was 4.25-6.00 us, DMA-to-PWM time was 20.72-21.66 us, and preload margin stayed at or above 31.09 us with zero rejection, missed updates, or faults; extend the same evidence through the remaining signed speed points |
| Current timing trace | 256 samples only after explicit active arm; timing reads dormant otherwise | Optional measurement contract; normal control avoids timing and per-event A/B reporting overhead while retaining all current, output, PWM, and deadline checks | ADC/current backend | Firmware 0.37.1 filled five sequential arms for 1,280 consecutive samples across signed velocity and scheduled-STOP runs. Every arm completed, timing improved relative to the matched 0.37.0 trace, and ordinary deadline/STOP release remained clean; worst-case concurrent-load evidence remains open |
