...
Status: firmware 0.38.0 / protocol 1.19 is the current source and flashed
...
| Quantity | Firmware 0.37.1 source value | Class and basis | Enforcement owner | Status / next evidence |
| --- | ---: | --- | --- | --- |
...
| Rotor/control release | 4 kHz, 250 us nominal | Implementation timing baseline with 16,000 core cycles between releases at 64 MHz | TIM6/TIM7, SPI1 DMA, PendSV, and `rotor_control_runtime` | The 0.38.0 simultaneous profile/current-trace gate completed 256/256 releases. Pend latency was 2.89/99.77 us average/maximum; PendSV work including current-loop preemption was 144.09/336.61 us. Repeat under other loads and retain stack high-water evidence before declaring WCET |
| Rotor publication | 56-byte progress at 4 kHz; 576-byte full state at 100 Hz plus transitions | Scheduling contract separating liveness/readiness fields from telemetry-sized controller state | `rotor_control_runtime` sequence-protected publications and foreground | Arm builds/static assertions confirm sizes; the 0.38.0 loaded profile includes publication at 13.31 us average and 185.52 us maximum, with broader load repetition still open |
| Foreground safety housekeeping | 1 kHz nominal | Implementation cadence for compact progress, liveness/readiness, runtime events, state invariants, RS-485 health, diagnostic deadline, and watchdog policy | Foreground main loop | The 0.38.0 loaded profile observed 61 passes averaging 256.35 us and peaking at 2.70 ms while host telemetry ran. Threshold crossings remain evaluated on the next due pass; characterize the long tail by foreground subsystem |
...
| Accepted feedback interval | 2,000 us maximum | Timing policy for the nominal 4 kHz encoder schedule, allowing eight nominal release periods before rejection | `aligned_torque_controller` | Firmware 0.38.0's simultaneous profiler gate held the host-captured interval to 250 us and the historical maximum to 254 us with no error or fault. Retain broader worst-case load measurement before tightening the 2,000 us bound |
| Fast electrical-phase prediction | Every 50 us; 3,000 us maximum observation age; 55 us measured output lead; observations through 20 rev/s | Timing contract: predict from regular-current DMA completion near 45 us to the 100 us PWM application boundary; the priority-1 guardian permits the intentional intervening update but faults a second update without new output | `electrical_phase_predictor` through `current_loop_backend` | The 0.38.0 simultaneous profile/current-trace gate retained 256 consecutive current samples, 6.75 us maximum trigger-to-DMA time, 21.81 us maximum DMA-to-PWM time, 29.59 us minimum preload margin, 525 us maximum prediction age, and zero rejection, missed updates, or faults |
| Current timing trace | 256 samples only after explicit active arm; timing reads dormant otherwise | Optional measurement contract; normal control avoids timing and per-event A/B reporting overhead while retaining all current, output, PWM, and deadline checks | ADC/current backend | Firmware 0.38.0 proved that this trace and the aggregate runtime profile can complete together without lost samples or missed PWM updates. The established sequential 0.37.1 evidence remains the disjoint baseline; repeat simultaneous capture at the remaining signed speed points |
| Aggregate runtime profile | 256 nominal 4 kHz releases only after explicit arm; 64 MHz DWT reads and accumulation dormant otherwise | Optional bufferless measurement contract for pend latency, deferred stages, total PendSV, foreground work, and higher-priority current completions | `runtime_profile`, SPI/PendSV, rotor runtime, and foreground | Firmware 0.38.0 completed 256/256 loaded releases at +4 rev/s. Total PendSV was 144.09/336.61 us average/maximum and foreground was 256.35/2,700.03 us; repeat across command, communications, display, error, and stack-high-water conditions |
