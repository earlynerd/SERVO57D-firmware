...
Status: firmware 0.37.0 / protocol 1.18 is the current source and flashed
...
| Encoder SPI clock | 8 MHz flashed setting | Exact divisor of the 32 MHz SPI1 peripheral clock; four-byte mode-3 DMA frame and 2 us CS setup/hold guards are retained | `spi1` | Firmware 0.37.0 signed torque, velocity, position, fault, and recovery gates ran with 250-258 us accepted intervals and zero reported encoder errors; formal SCK/CS signal-integrity correlation, parity/status behavior, and long-run error counters remain open |
| Rotor/control release | 4 kHz, 250 us nominal | Implementation timing candidate with 16,000 core cycles between releases at 64 MHz | TIM6/TIM7, SPI1 DMA, PendSV, and `rotor_control_runtime` | The -4 rev/s gate held captured intervals to 250-251 us and prediction age to 174-425 us while an armed 20 kHz trace remained consecutive; formal PendSV WCET, higher-priority current-loop preemption, foreground load, and stack high-water evidence remain open |
...
| Accepted feedback interval | 2,000 us maximum | Timing policy for the nominal 4 kHz encoder schedule, allowing eight nominal release periods before rejection | `aligned_torque_controller` | Firmware 0.37.0 signed torque, velocity, and position gates held accepted intervals to 250-258 us; retain formal preemption and worst-case load measurement before tightening the 2,000 us bound |
| Fast electrical-phase prediction | Every 50 us; 3,000 us maximum observation age; 55 us measured output lead; observations through 20 rev/s | Timing contract: predict from regular-current DMA completion near 45 us to the 100 us PWM application boundary; the priority-1 guardian permits the intentional intervening update but faults a second update without new output | `electrical_phase_predictor` through `current_loop_backend` | Firmware 0.37.0's -4 rev/s trace measured 174-425 us prediction age, 4.25-4.31 us trigger-to-DMA time, 22.47-22.97 us DMA-to-PWM time, and at least 29.78 us preload margin with zero rejection, missed updates, or faults; extend the same evidence through the remaining signed speed points |
| Current timing trace | 256 samples only after explicit active arm; timing reads dormant otherwise | Optional measurement contract; normal control avoids TIM2/DWT/preload timing overhead while retaining all current, output, PWM, and deadline checks | ADC/current backend | The firmware 0.37.0 -4 rev/s arm filled all 256 consecutive samples and preserved clean STOP/deadline release; formal disarmed ISR timing, repeated-arm behavior, and worst-case concurrent-load evidence remain open |
...
| Direct velocity-reference acceleration | Caller-selected positive value through 256 rev/s²; host and legacy default 16 rev/s² | Explicit trajectory shape; 256 retains fourfold headroom over the maximum position-profile acceleration and is controller capability, not a motor claim | Command service and `velocity_controller` | Flashed 0.37.0 reached ±4 rev/s through 16 rev/s² launches with 0.073/0.132 rev/s RMS error, 26/61 counts peak q-current request, clean timing, and no faults; the attached motor had audibly skipped at a 32 rev/s² position profile, which remains motor-specific evidence rather than a firmware ceiling |
...
| Velocity feedback interval | 2,000 us maximum | Same timing policy as the 4 kHz aligned actuator | `velocity_controller` and `aligned_torque_controller` | The firmware 0.37.0 -4 rev/s run captured 250-251 us intervals with no rejection or fault; retain formal worst-case preemption/load qualification |
